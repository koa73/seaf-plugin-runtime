...
def main() -> int:
    try:
        ...
        _log_items("specific_add", event, items)
        ...
        _emit_error(f"specific_add failed: {exc}")
        return write_response(status="error", message=f"specific_add failed: {exc}", payload={"handler": "specific_add"})
