...
def main() -> int:
    try:
        ...
        _log_items("all_remove", event, items)
        return write_response(
            status="success",
            message=f"all_remove processed: {len(items)}",
            payload={"handler": "all_remove", "count": len(items)},
        )
    except Exception as exc:
        _emit_error(f"all_remove failed: {exc}")
        return write_response(status="error", message=f"all_remove failed: {exc}", payload={"handler": "all_remove"})
