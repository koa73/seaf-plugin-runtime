...
- Печатает явные фазы выполнения (`create-or-reuse-venv`, `upgrade-pip`, `install-requirements`, `preflight-import`) и tail ошибок.
...
При неуспехе скрипт выводит:
- последнюю выполненную команду;
- stderr-tail (последние строки);
- короткую подсказку по fallback без sudo.
