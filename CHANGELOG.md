...
- **Root cause**: ранее высота ограничивалась `Math.min(380, ...)` с недостаточной компенсацией (`+44`), из-за чего контент с кнопками визуально прижимался к нижнему краю даже при установленном padding.
- **Layout tuning**: диапазон высоты и frame-компенсация увеличены (`min/max` и `dialogFrameCompensation`) для стабильного отображения нижнего отступа.
...
- **Height adjustment**: увеличен диапазон контент-ориентированного расчета высоты диалога, чтобы новый нижний отступ стабильно отображался без обрезки.
