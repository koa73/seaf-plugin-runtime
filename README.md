...
- Команда открывает вспомогательное окно параметров (`Источник`, `Приемник`, `Тип связи`, `Геометрия линии`, `Цвет`, `Тип линии`, `Тип стрелки`, `label`), где endpoint-пункты отображаются как `OID (title)`; `label` опционален, `source != target` обязателен.
- Связь создается через стандартный draw.io API `graph.insertEdge`, итоговый стиль формируется строго из текущих значений окна (стартовые дефолты или изменённые пользователем параметры), а в данные edge записывается `schema=seaf.company.ta.services.logical_links`.
