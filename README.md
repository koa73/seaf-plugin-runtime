...
- Смена родителя в модели эмитится как **`reparent`** → `seafStencilReparent` (**без** OID и **без** UI-команд): `events/reparent.py` пишет в лог **`reparentScriptFired`** и полный **`event`** из payload; слой группы задаётся на **`add`**. Для строки в `seaf-plugin.log` нужны **`pluginLogLevel`** (payload) и **`scriptLogLevel`** (`env.yaml`) не ниже `info` — см. `CHANGELOG` 0.5.27–0.5.28.
