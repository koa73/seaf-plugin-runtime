...
- Смена родителя в модели эмитится как **`reparent`** (не `add`) и маршрутизируется в `seafStencilReparent` / `events/reparent.py`: без назначения OID; семантический слой из `schemas.<schema>.layer` не меняется — вложение иерархии слоёв обновляется командой **`moveLayerUnderLayer`** (`suppressStencilEvents: true`), а не переносом объектов в другой семантический слой через `moveObjectsToLayer`.
...
- Добавлена команда `moveObjectsToLayer`: create-or-get слоя + перенос указанных объектов в слой через `graph.moveCells(...)` (в т.ч. для `add` и mirror-потоков); для ячеек, уже находящихся на слое с тем же именем, перенос не выполняется (и Python layer-routing не включает их в `objectIds`, если в item задан `currentLayerName`).
- Добавлена команда `moveLayerUnderLayer`: перенос mxCell **семантического слоя** (имя из `schemas.<schema>.layer`) под **другой страничный слой** (`parentLayerName`); используется для `reparent` (двухуровневая модель слоёв), с `suppressStencilEvents` по необходимости.
