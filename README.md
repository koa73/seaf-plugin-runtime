...
- Смена родителя в модели эмитится как **`reparent`** (не `add`) и маршрутизируется в `seafStencilReparent` / `events/reparent.py`: **без** назначения OID и **без** UI-команд слоя — семантический слой из `schemas.<schema>.layer` задаётся только на **`add`** (`moveObjectsToLayer` в `all_add`), при `reparent` дерево слоёв не меняется.
