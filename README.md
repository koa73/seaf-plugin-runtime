...
- Команда открывает вспомогательное окно параметров (`Источник`, `Приемник`, `Тип связи`, `Геометрия линии`, `Цвет`, `Тип линии`, `Тип стрелки`, `label`, `Описание`), где endpoint-пункты отображаются как `OID (title)`; `label` и `Описание` опциональны, `source != target` обязателен.
