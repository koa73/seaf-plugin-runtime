...
- Команда открывает вспомогательное окно параметров (`Источник`, `Приемник`, `Тип связи`, `Цвет`, `Тип линии`, `Тип стрелки`) с валидацией `source != target`; кнопка `Создать` активна только при полном валидном заполнении.
- Связь создается через стандартный draw.io API `graph.insertEdge`, а итоговый стиль формируется строго из текущих значений окна (стартовые дефолты или изменённые пользователем параметры).
