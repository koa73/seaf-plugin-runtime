...
- Смена родителя в модели эмитится как **`reparent`** → `seafStencilReparent` (**без** OID и **без** UI-команд): `events/reparent.py` только пишет **аудит** в лог (`reparent_move_audit`: объекты и `currentLayerName`); слой группы задаётся на **`add`**. Для строки в `seaf-plugin.log` нужны **`pluginLogLevel`** (payload) и **`scriptLogLevel`** (`env.yaml`) не ниже `info` — см. `CHANGELOG` 0.5.27.
